...
export default {
  ...
  created () {
    ...
    this.$http.get('http://127.0.0.1:8080/static/goods.json', {})
      .then(res => {
        // console.log(res)
        if (res.data.errno === 0) {
  ...
  methods:{
    initScroll(){
      ...
      this.foodsScroll.on('scrollEnd',(pos)=>{
        // console.log(pos.y)//<0
        var ls=this.listHeght
        for(let i=0;i<ls.length-1;i++){
          var h1=ls[i],h2=ls[i+1]
          if(pos.y>-h2&&pos.y<=-h1){
            // console.log(i)
    ...
    selectMenu(index,event){
      ...
      // console.log(index)
      this.foodsScroll.scrollTo(0,-this.listHeght[index],700,{easing:"bounce"})
    },
    calculateHeight(){
